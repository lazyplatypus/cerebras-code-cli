...
const image = "ghcr.io/kevint-cerebras/cerebras-cli"
...
if (!Script.preview) {
  ...
  for (const [pkg, pkgbuild] of [["cerebras-bin", binaryPkgbuild]]) {
    for (let i = 0; i < 30; i++) {
      try {
  ...
  const tap = `https://x-access-token:${token}@github.com/kevint-cerebras/homebrew-tap.git`
